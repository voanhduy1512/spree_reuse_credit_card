...
describe "PayWithCreditCards" do
  describe "GET /checkout/payment" do
    ...
    before(:each) do
      @bogus_payment_method = FactoryGirl.create(:bogus_payment_method)
    ...
    context "existing cards" do
      before(:each) do
        ...
        @credit_card = FactoryGirl.create(:credit_card, :gateway_payment_profile_id => random)
        ...
        # add a payment
        payment = FactoryGirl.create(:payment, :order => order, :source =>  @credit_card, :amount => order.total, :payment_method => @bogus_payment_method, :state => 'completed')
